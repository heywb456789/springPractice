...
public class AuthController {
    ...
    @PostMapping("/login")
    public Mono<ResponseDTO<?>> login(@RequestBody AuthRequestDTO req, HttpServletRequest servletRequest) {
    ...
    public ResponseDTO<AuthResponseDTO> register(
        ...
        OneIdResponse resp = tomatoService.authenticate(req.getPhoneNumber(), req.getPassword())
                .block(); // 실서비스에서는 블로킹 없이 처리 권장
    ...
    public ResponseDTO<?> logout(@AuthenticationPrincipal MemberUserDetails userDetails, HttpServletRequest request) {
