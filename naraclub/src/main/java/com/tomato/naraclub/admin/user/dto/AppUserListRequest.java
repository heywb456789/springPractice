...
import com.tomato.naraclub.admin.user.code.UserSearchType;
 ...
 * @fileName : AppUserListRequest
 * @date : 2025-05-12
 * @description : 앱 사용자 목록 요청 DTO
 * @AUTHOR : MinjaeKim
 */
@Slf4j
@Getter
@Setter
@ToString
public class AppUserListRequest implements SearchTypeRequest {
    ...
    @Schema(description = "정렬 기준: LATEST, NAME, LOGIN")
    private AppUserSortType sortType;

    @Schema(description = "정렬 방향: ASC, DESC")
    private Order sortDirection;

    @Schema(description = "날짜 범위 (yyyy-MM-dd ~ yyyy-MM-dd)")
    private String dateRange; // 프론트에서 'dateRange' 파라미터로 전달
    ...
    public OrderSpecifier<?> getSortOrder() {
        AppUserSortType st = (this.sortType != null ? this.sortType
            : AppUserSortType.LATEST);
    ...
    @Hidden
    public BooleanExpression getMemberStatusCondition(QMember member) {
